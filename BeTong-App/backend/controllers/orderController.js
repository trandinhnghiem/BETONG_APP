...
class OrderController {
  ...
  static async createOrder(req, res) {
    try {
      ...
      res.json({ message: 'Tạo đơn thành công', orderId, orderCode })
  ...
  static async updateStatus(req, res) {
    try {
      ...
      const validStatuses = [
        ...
        'Uploading','Sent','Delivered','Completed'
      ]
      ...
      const success = await OrderModel.updateStatus(orderId, status, req.user.Id)
  ...
  static async confirmPayment(req, res) {
    try {
      res.json({ message: 'Đã xác nhận thanh toán (demo)' })
    } catch (error) {
      res.status(500).json({ error: error.message })
    }
  }
