...
async function ensureUserStationColumn() {
  try {
    ...
    const result = await pool.request().query(`
      SELECT 1
      FROM sys.columns
      WHERE object_id = OBJECT_ID('Users') AND name = 'StationId'
    `)

    if (result.recordset.length === 0) {
      await pool.request().query(`ALTER TABLE Users ADD StationId INT NULL`)
      console.log('✅ Added StationId column to Users table')
    }
  } catch (error) {
    console.error('Error ensuring StationId column:', error.message)
  }
}
...
async function seedStations() {
  try {
    ...
    const stationDefinitions = [
      { stationCode: 'OMON1', stationName: 'Trạm Ô Môn 1', address: 'Ô Môn, Cần Thơ', phone: '0900000001' },
      { stationCode: 'OMON2', stationName: 'Trạm Ô Môn 2', address: 'Ô Môn, Cần Thơ', phone: '0900000002' },
      { stationCode: 'T82', stationName: 'Trạm T82', address: 'Tây Đô', phone: '0900000003' },
      { stationCode: 'HAUGIANG', stationName: 'Trạm Hậu Giang', address: 'Hậu Giang', phone: '0900000004' }
    ]

    const stations = {}
    for (const data of stationDefinitions) {
      let station = await StationModel.findByCode(data.stationCode)
      if (!station) {
        station = await StationModel.create(data)
        console.log(`✅ Created station: ${data.stationName}`)
      }
  ...
  try {
    ...
    for (const user of stationUsers) {
      ...
      if (!existing) {
        ...
        console.log(`✅ Station user created: ${user.username} (${user.fullName})`)
      } else if (!existing.StationId) {
        await UserModel.updateStationId(existing.Id, station.Id)
        console.log(`✅ Updated station assignment for user: ${user.username}`)
      }
    }
  } catch (error) {
